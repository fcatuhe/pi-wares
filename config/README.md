...
| [`pi/pi-codex-subagents/agents/rails-review.md`](./pi/pi-codex-subagents/agents/rails-review.md) | `~/.pi/agent/pi-codex-subagents/agents/rails-review.md` | The `rails-review` subagent: read-only tools, the `rails-review` skill, the prompt every spawn of it starts with. Templates load from that directory and nowhere else. Copy it by hand for now, the doctor reconciles JSON and TOML, not markdown. |
