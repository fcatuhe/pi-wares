...
| [`pi/pi-codex-subagents/agents/rails-review.md`](./pi/pi-codex-subagents/agents/rails-review.md) | `~/.pi/agent/pi-codex-subagents/agents/rails-review.md` | The `rails-review` subagent: read-only tools, the `rails-review` skill, its prompt, and Opus at high thinking so the reviewer is not whatever the caller was running. A caller's own `model` and `thinking` still win. Templates load from that directory and nowhere else, and the doctor reconciles JSON and TOML, not markdown, so copy this one by hand. |
