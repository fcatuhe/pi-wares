...
|---|---|---|
| [`pi/settings.json`](./pi/settings.json) | `~/.pi/agent/settings.json` | `enabledModels` is the one list both `model-shortcuts` and subagent routing read, so an empty one makes both look broken. The model and thinking defaults are house taste, as is `tuiMode: "fullscreen"`, which leaves the transcript scrollbar on its `auto` default. `showCacheMissNotices: true` surfaces prompt-cache misses in the transcript instead of hiding what a long session is costing. Add a model here and the doctor asks for it; anything extra you enabled yourself stays. |
