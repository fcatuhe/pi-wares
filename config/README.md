...
| [`pi/extensions/pi-model-shortcuts.json`](./pi/extensions/pi-model-shortcuts.json) | `~/.pi/agent/extensions/pi-model-shortcuts.json` | One slash shortcut per enabled model, in the same order as `enabledModels`. |
| [`pi/pi-codex-subagents/config.json`](./pi/pi-codex-subagents/config.json) | `~/.pi/agent/pi-codex-subagents/config.json` | Without `modelsFromEnabledModels`, the subagents extension silently drops its per-spawn `model` argument. The failure mode that made this directory exist. `defaults.extensions` is the second one: a spawn starts with `--no-extensions`, so without it every subagent writes code under no house policy. |
| [`pi/pi-codex-subagents/agents/rails-review.md`](./pi/pi-codex-subagents/agents/rails-review.md) | `~/.pi/agent/pi-codex-subagents/agents/rails-review.md` | The `rails-review` subagent: read-only tools, the `rails-review` skill, its prompt, and a pinned model at high thinking so the reviewer is not whatever the caller was running. A caller's own `model` and `thinking` still win. Templates load from that directory and nowhere else, and the doctor reconciles JSON and TOML, not markdown, so copy this one by hand. |
...
`~/.pi/agent` follows `PI_CODING_AGENT_DIR` and `~/.config` follows `XDG_CONFIG_HOME` when either is set. The subagent extension paths are the exception: they are values inside a file, not target paths, so they spell out `~/.pi/agent/git/github.com/fcatuhe/pi-wares/...`, the clone `pi install` makes. Move the agent directory and those two lines need editing by hand, or a spawn fails naming the path it could not find.
