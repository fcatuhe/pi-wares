...
`/preview:md` previews a markdown file in a herdr split beside pi, rendered and live.

```text
/preview:md                    # pick a file
/preview:md docs/pricing.md    # open one, with argument completion
/preview:md auto               # toggle preview-on-write
```

The command is namespaced by format, the way pi names `/skill:x`. Another viewer would land as `/preview:pdf` rather than as a second top-level verb.
...
No project-local override: a repo that set `auto` would silently outrank `/preview:md auto`.
