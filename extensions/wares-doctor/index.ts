...
export default function (pi: ExtensionAPI) {
  pi.registerEntryRenderer<Report>(REPORT_ENTRY, (entry, _options, theme) => {
    ...
    const line = (row: Row) =>
      typeof row === "string" ? row : `${row.label}  ${row.tone ? theme.fg(row.tone, row.state) : row.state}  ${row.path}${row.hint}`;
    const body = [theme.fg("accent", `/${command}`), ...rows.map(line), ...notes.map((note) => theme.fg(note.tone, note.text))].join("\n");
