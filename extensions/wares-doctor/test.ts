...
const REFERENCE_JSON = `{"defaultModel":"claude-opus-5","defaultThinkingLevel":"high","enabledModels":["anthropic/claude-opus-5"]}`;

const ROOT = join(import.meta.dirname, "..", "..");
const RAILS_REVIEW = "pi-codex-subagents/agents/rails-review.md";

const toml = reconcileToml(USER_TOML, REFERENCE_TOML);
...
let home = "";
let settings = "";
let kept: Report["notes"] = [];

test("TOML regions the reference does not touch stay byte-identical, comments and alignment included", () => {
...
test("missing TOML tables arrive whole, with the comment above them", () => {
...
test("several keys landing in one TOML table keep the reference order", () => {
  const ordered = reconcileToml(`[keys]\nprefix = "ctrl+a"\n`, `[keys]\nprefix = "x"\nfirst = "1"\nsecond = "2"\n`);
  assert.match(ordered.text, /first = "1"\nsecond = "2"/, "inserted keys came out reversed");
});

test("each TOML key is found diverged, ok or missing against the reference", () => {
...
test("a second TOML pass adds nothing and rewrites nothing", () => {
  const again = reconcileToml(toml.text, REFERENCE_TOML);
...
test("alternate chords land whole when absent, and a lone string set by the user keeps it and gains ours", () => {
  ...
  assert.equal(reconcileToml(chords.text, ALTERNATES_TOML).text, chords.text, "applying twice is not idempotent");
});

test("absent root keys land in the root table, not inside the last table", () => {
  const rootless = reconcileToml(`[keys]\nprefix = "ctrl+space"\n`, REFERENCE_TOML);
  assert.match(rootless.text, /^onboarding = false\n\[keys\]/, "a root key was written into a table");
});

test("TOML force makes the reference win on the keys it declares, and only those", () => {
  const forced = reconcileToml(USER_TOML, REFERENCE_TOML, true);
  ...
  assert.equal(reconcileToml(forced.text, REFERENCE_TOML, true).text, forced.text, "forcing twice is not idempotent");
});

test("a key the reference sets inside a table the user never opened is still manual under force", () => {
...
test("JSON gains the reference keys and array members, never replacing what is set", () => {
  const json = reconcileJson(USER_JSON, REFERENCE_JSON);
  ...
  assert.equal(reconcileJson(json.text, REFERENCE_JSON).text, json.text, "applying twice is not idempotent");
});

test("JSON force replaces the user's value, and extra array members are additions, not disagreements", () => {
  const forcedJson = reconcileJson(USER_JSON, REFERENCE_JSON, true);
...
test("a bare machine reports every target as a warning to create, one row per file", () => {
  ...
  assert.deepEqual(
    ...
    ["warning", "text", "text", "text", "text", "text"],
    "the notes changed tone",
  );
  assert.deepEqual(
    bare.notes.map((note) => note.text),
    ["5 to add. /wares-doctor:apply writes them.", "  pi settings", "  model shortcuts", "  subagents", "  rails-review agent", "  herdr"],
...
test("apply creates every target as the reference, then nothing is left to report", () => {
  ...
  for (const file of ["agent/settings.json", "agent/model-shortcuts/config.json", `agent/${RAILS_REVIEW}`, "config/herdr/config.toml"]) {
...
test("a diverged key is reported with both values, kept as a warning, and force is offered", () => {
  settings = join(home, "agent/settings.json");
  writeFileSync(settings, readFileSync(settings, "utf-8").replace(`"high"`, `"low"`));
  const diverging = report("");
  kept = diverging.notes;
  assert.deepEqual(
...
test("apply keeps the diverged value and still says so", () => {
...
test("force says what it replaced, has nothing left to keep, and a second force is quiet", () => {
...
test("a machine one key and one model short lists each key with the value that would land", () => {
...
test("a value parsed but not written as a plain key is manual, an error no command can close", () => {
...
test("a diverged rails-review agent is kept by report and apply, and replaced whole by force", () => {
  const agentHome = bareMachine();
  report(APPLY);
  const agentFile = join(agentHome, "agent", RAILS_REVIEW);
  const reference = readFileSync(join(ROOT, "config/pi", RAILS_REVIEW), "utf-8");
  const mine = `${reference}\nMy own review rules.\n`;
  writeFileSync(agentFile, mine);
  const expectedNotes = [
    { tone: "warning", text: `1 kept as yours. /wares-doctor:${FORCE} takes the reference instead.` },
    { tone: "text", text: "  rails-review agent differs from the reference" },
  ];

  const reported = report("");
  assert.deepEqual(reported.notes, expectedNotes, "a diverged agent file is not reported as kept");
  assert.equal(reported.rows[3].state.trim(), "kept 1");
  assert.equal(reported.rows[3].tone, "warning", "a diverged agent file renders like one that matches");
  assert.equal(readFileSync(agentFile, "utf-8"), mine, "a report wrote the agent file");

  assert.deepEqual(report(APPLY).notes, expectedNotes, "apply dropped the kept note");
  assert.equal(readFileSync(agentFile, "utf-8"), mine, "apply overwrote the user's agent file");

  const forced = report(FORCE);
  assert.equal(forced.rows[3].state.trim(), "replaced 1");
...
test("the command appends one report entry naming the command that produced it", () => {
  bareMachine();
...
test("anything typed after the command is a mistake worth saying out loud, and runs nothing", () => {
...
test("a target directory that cannot exist is reported as a failure, never a blank card", () => {
