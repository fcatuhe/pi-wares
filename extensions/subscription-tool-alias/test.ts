...
import { buildAliasCandidates, createAliasMaps, namespaceFrom, rewritePromptText, transformPayload, unaliasToolCalls } from "./index.ts";
...
test("the namespace is the package dir minus its pi- prefix, sanitized, with wrapper dirs skipped", () => {
...
test("path wins over baseDir, which can be a monorepo root", () => {
  assert.equal(namespaceFrom({ path: "/repo/packages/pi-foo/index.ts", baseDir: "/repo" }), "foo");
});

test("builtins have no source dir and get local, since a namespace naming pi would undo the prompt neutralization", () => {
  assert.equal(namespaceFrom({ path: "<builtin:ls>" }), "local");
  assert.equal(namespaceFrom(undefined), "local");
});

test("every registered tool but a real mcp__ one gets a candidate, applied per payload", () => {
...
test("a name the provider would reject as an alias is left without one", () => {
  assert.equal(buildAliasCandidates([{ name: "foo.bar", sourceInfo: { path: "/x/pi-a/i.ts" } }]).size, 0);
});

test("a tool named as the transport spells it opts out, since canonicalized it misses the candidate map", () => {
  ...
  assert.deepEqual(
    (firstParty.tools as Array<{ name: string }>).map((tool) => tool.name),
    ["WebSearch"],
  );
});

test("names differing only in case are separate tools, each aliased", () => {
...
test("over the 128-char limit the namespace is truncated, never the flat name that routes the call back", () => {
  const longName = "a".repeat(100);
  const longAlias = buildAliasCandidates([{ name: longName, sourceInfo: { path: "/x/pi-a-very-long-package-name/i.ts" } }]).get(longName);
...
test("the Available tools declarations follow the wire name, prose mentioning a tool stays as written", () => {
  assert.equal(
...
test("tools are aliased with their metadata, canonicalized and native ones passing through", () => {
  assert.deepEqual(
...
test("descriptions are never rewritten, aliased tool or native typed one", () => {
  assert.equal(outTool("mcp__codex_subagents__spawn_agent").description, "Use `wait_agent` or wait_all_agents only when needed.");
  assert.equal(outTool("web_search").description, "see spawn_agent");
});

test("tool_choice and history are aliased, and the original payload is untouched", () => {
...
test("transforming the transformed payload changes nothing", () => {
  assert.deepEqual(transformPayload(out, candidates, maps), out);
});

test("a real tool already advertising the derived alias blocks the mapping, so its calls are never hijacked", () => {
...
test("aliased calls return to their flat names in place, foreign mcp and canonicalized names are left alone", () => {
  const assistant = {
    role: "assistant",
    content: [
...
test("nothing to rename reports false, so the message_end hook leaves the message alone", () => {
  assert.equal(unaliasToolCalls({ role: "assistant", content: [{ type: "text", text: "hi" }] }, maps), false);
});

test("a non-assistant message carrying an alias is not ours to rewrite", () => {
  const userMessage = {
    role: "user",
...
test("a canonicalized name is left alone in history for an unadvertised tool, and what goes on the wire comes back", () => {
...
test("client-name phrases from the stock prompt are neutralized", () => {
  assert.equal(
...
// A general \bpi\b pass once turned these into nonsense.
test("words that are not the product name survive the neutralization", () => {
  const unrelated = [
