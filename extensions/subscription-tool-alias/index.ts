...
export function transformPayload(raw: Record<string, unknown>, candidates: Map<string, string>, maps: AliasMaps): Record<string, unknown> {
  const payload = { ...raw };
  ...
  if (isPlainObject(payload.tool_choice) && payload.tool_choice.type === "tool" && typeof payload.tool_choice.name === "string") {
