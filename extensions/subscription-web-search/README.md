...
Only `http` and `https` are fetchable. Private addresses are not blocked, deliberately: this repo's sessions have `bash`, so an SSRF guard here would protect nothing while pretending otherwise.

Fetches stop at 5MB, read through the stream rather than after buffering, and time out at 20s. Extracted markdown is capped at 300k characters, and the result says so when it was cut. API calls time out at 120s.
