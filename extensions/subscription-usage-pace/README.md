...
The weekly per-model cap (`weekly_scoped`) is **not** shown, two bars are all the footer fits: a green `7d` does not promise the model you are on has weekly quota left. The sibling `five_hour` / `seven_day` objects are ignored, their float `utilization` has no distinguishable unit below 1 (`0.6` is 0.6%, not 60%).
...
- **Only a read that produced windows holds the slot.** A failed poll stakes `polledAt` too, so counting it would hand the next session an empty cache, no bar, and no poll of its own for five minutes: the extension reads as broken for exactly as long as the failure is invisible. An empty cache is always pollable, and the 429 hold still gates it.
