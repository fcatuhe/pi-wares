	...
	pi.registerTool(
		defineTool({
			...
			parameters: Type.Object({
				...
				vars: Type.Optional(Type.Object({}, { additionalProperties: Type.String(), description: "Values for the {placeholders} of a preset" })),
				...
				keys_fallback: Type.Optional(
					Type.Boolean({ description: "For an agent without a radio (claude, codex): type the call into its terminal instead. No reply comes back." }),
				),
			}),
			...
			async execute(_toolCallId, params, signal) {
				...
				if (peer.reach === "keys" || !peer.session_id) {
					...
					return {
						content: [{ type: "text", text: `Typed into ${peer.name} (${peer.pane_id}) as its owner. No acknowledgement and no reply are possible on that path.` }],
						details: envelope,
					};
				}
				...
				if (!answered) {
					return {
						content: [{ type: "text", text: `${landed} No answer within ${Math.round(REPLY_TIMEOUT_MS / 1000)}s. It may still answer later, as a call of its own.` }],
						details: envelope,
					};
				}
